...
while (<$fp>) {chomp;
	...
	#print STDERR "READ A frequency of ".$freq."\n";
...
for (my $gen = 1; $gen <= $gen_count; $gen++) {
	...
	#var_count and all_vars are both 0!
	my $gen_scores = get_random_vars ($var_count, $all_vars );	#selects a subset of all variants for use in analysis
	print STDERR "loop throgh ".$#$samples." samples. I'll print a dot every time\n";
...
sub compare_genotypes { my ($gen_scores, $sample1_id, $sample2_id, $shares) = @_;
	...
	#this loop never happens - gen_scores is empty?
	#print "i've got ".$#{$gen_scores->{$sample1_id}}." score for sample1=".$sample1_id."\n";
	for (my $i = 0; $i <= $#{$gen_scores->{$sample1_id}}; $i++ ) {		#looping through genotypes of sampled variants
		...
		#print "GEN are ".$sample1_gen." ".$sample2_gen."\n";
...
sub store_share_results { my ($shares, $count, $shared, $id1, $id2) = @_;
	#print STDERR "count is zero for ".$id1." to ".$id2."\n";
	if ( $count >0 ) {
		#	print STDERR "for ".$id1." and ".$id2." I have ".$shared." shared out of ".$count."\n";
...
sub get_random_vars { my ($var_count, $all_vars) = @_;
	my $gen_scores = {};
	#	print STDERR "I want ".$var_count." out of ".@{$all_vars}."\n";
...
sub getGenSum { my $var_calls = shift;
	...
	#this is the linve I've modified to allow phased genotype calls, alhtough phase is ignored here!
	#JAC 1/4/21;
